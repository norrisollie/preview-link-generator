...
function setupDom() {
    ...
    app.dom.previewLinksButton = document.getElementById("preview-links-button");
    app.dom.floatingContainer = document.getElementById("floating-container");
    app.dom.previewSectionButtons = document.querySelectorAll(".preview-section-buttons");
    app.dom.jsonLinkCheckboxes = document.querySelectorAll(".json-link-checkbox");
    app.dom.deselectAllLinks = document.getElementById("preview-links-deselect");
    app.dom.selectAllLinks = document.getElementById("preview-links-select");
    app.dom.previewLinksCopy = document.getElementById("preview-links-copy");
    app.dom.previewLinksOpen = document.getElementById("preview-links-open");
    app.dom.previewLinksPreview = document.getElementById("preview-links-preview");
    app.dom.jsonLinksTextarea = document.getElementById("json-links-textarea");
...
function addListeners() {
    ...
    app.dom.previewLinksButton.addEventListener("click", previewButtonClickHandler);

    for (var i = 0; i < app.dom.previewSectionButtons.length; i++) {
        app.dom.previewSectionButtons[i].addEventListener("click", previewSectionButtonsClickHandler)
    }

    for (var i = 0; i < app.dom.jsonLinkCheckboxes.length; i++) {
        app.dom.jsonLinkCheckboxes[i].addEventListener("input", jsonLinkCheckboxesChangeHandler)
    }


}

function previewButtonClickHandler(e) {

    var targetState = e.currentTarget.dataset.state;

    if (targetState === "closed") {

        app.dom.previewLinksButton.dataset.state = "open";
        app.dom.previewLinksButton.classList.remove("preview-button");
        app.dom.floatingContainer.style.transform = "none";

    } else if (targetState === "open") {

        app.dom.floatingContainer.style.transform = "translateY(100%)";

        setTimeout(function() {
            app.dom.previewLinksButton.dataset.state = "closed";
            app.dom.previewLinksButton.classList.add("preview-button");
        }, 500);
    }
}

function previewSectionButtonsClickHandler(e) {
    ...
    switch (targetAction) {
        ...
        case "select":
            ...
            for (var i = 0; i < app.dom.jsonLinkCheckboxes.length; i++) {
                ...
                app.dom.jsonLinkCheckboxes[i].checked = true;

            }

            app.dom.deselectAllLinks.removeAttribute("disabled");
            app.dom.selectAllLinks.setAttribute("disabled", "true");

            app.dom.previewLinksCopy.removeAttribute("disabled");
            app.dom.previewLinksOpen.removeAttribute("disabled");
            app.dom.previewLinksPreview.removeAttribute("disabled");
        ...
        case "deselect":
            ...
            app.dom.selectAllLinks.removeAttribute("disabled");
            app.dom.deselectAllLinks.setAttribute("disabled", "true");

            app.dom.previewLinksCopy.setAttribute("disabled", "true");
            app.dom.previewLinksOpen.setAttribute("disabled", "true");
            app.dom.previewLinksPreview.setAttribute("disabled", "true");

            for (var i = 0; i < app.dom.jsonLinkCheckboxes.length; i++) {

                app.dom.jsonLinkCheckboxes[i].checked = false;
        ...
        case "copy":
            ...
            for (var i = 0; i < app.dom.jsonLinkCheckboxes.length; i++) {
                ...
                if (app.dom.jsonLinkCheckboxes[i].checked === true) {

                    var fullUrl = app.dom.jsonLinkCheckboxes[i].parentNode.parentNode.dataset.fullurl

                    app.dom.jsonLinksTextarea.value += fullUrl + "\n";

                    app.dom.jsonLinksTextarea.select();
        ...
        case "preview":
            ...
            for (var i = 0; i < app.dom.jsonLinkCheckboxes.length; i++) {
                ...
                if (app.dom.jsonLinkCheckboxes[i].checked === true) {
                    ...
                    var parent = app.dom.jsonLinkCheckboxes[i].parentNode.parentNode
                    var width = parent.dataset.width
                    var height = parent.dataset.height

                    var preview = document.createElement("div");
                    preview.classList.add("ad-preview");
                    parent.appendChild(preview);

                    preview.innerHTML += "<iframe src='" + fullUrl + "' width='" + width + "' height='" + height + "'></iframe>"
...
function jsonLinkCheckboxesChangeHandler(e) {
    ...
    for (var i = 0; i < app.dom.jsonLinkCheckboxes.length; i++) {
        ...
        if (app.dom.jsonLinkCheckboxes[i].checked === true) {
            ...
            console.log(app.dom.jsonLinkCheckboxes[i])

        }

    }

    if (count >= 1 && count < app.dom.jsonLinkCheckboxes.length) {
    ...
    } else if(app.dom.jsonLinkCheckboxes.length == count) {
